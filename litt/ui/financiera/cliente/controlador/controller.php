...
if(isset($g['listarClientes'])){
    ...
    // el proceso
    $q = "select * from clientes where observaciones is not null";
    $ret = query_ret($q);
    $arr_clientes = array();
...
if(isset($g['eliminar'])){
    $q = "delete from clientes where dni = '" . $g['eliminar'] . "'";
    // echo $q;
    query_noret($q);
...
if(isset($g['guardarDatos1'])){
    ...
    query_noret($q);
    // creo el credito
    $x = "insert into credito_instancia (dni_cliente,monto,id_plan) values ("
            . "'" . $p['dni'] . "',"
            . "'" . $p['monto'] . "',"
            . "'" . $p['plan'] . "')";
    // echo $x;
...
if(isset($g['guardarDatos3'])){
    $q = "update clientes set"
        ...
        . " where dni = '" . $p['dni'] . "'";
    query_ret($q);
    header('Location: ../vista/alta_7_altaDefinitiva.php');   
}
